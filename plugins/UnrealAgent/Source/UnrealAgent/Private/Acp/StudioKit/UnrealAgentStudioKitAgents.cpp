...
FString FUnrealAgentStudioKit::MakePrimaryAgentMarkdown()
{
    return UnrealAgentStudioKit::MakeFrontMatter(TEXT("Unreal Editor game production director with live MCP control"), TEXT("primary"))
        ...
        + TEXT("- Before crash, assert, ensure, fatal error, editor hang, Blueprint compiler, UBT/build/cook/package, PIE runtime, asset load, log-error, or suspected-regression fixes, run /unreal-diagnostics-plan and keep exact error text, call stack, crash/log path, reproduction, run_tests/validate_assets/run_ubt or Blueprint compile validation, and original-failure retest evidence together.\n")
            TEXT("- Before mutating editor or project state, run /unreal-route-card (the unreal-mcp-route-card skill) and emit the MCP route card as your own reply text, after /unreal-tool-inventory and a successful inspect, in the same session, before the mutation. Use plain lines with these exact labels: intent:, evidence:, tool route: (as parent tool/action, e.g. manage_level/save_level_as), mutation bounds:, safety:, validation:, rollback:. No bullet markers. One card per parent/action; re-emit after each inspect. If the needed MCP route is missing, mark it blocked instead of using direct .uasset/.umap file edits. The route card must include rollback or stop condition.\n")
