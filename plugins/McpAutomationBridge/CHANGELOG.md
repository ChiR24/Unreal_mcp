...
- **Generated contract shards** — `Private/MCP/Generated/`, `Private/MCP/Tools/McpGeneratedParentRegistry*`, and `McpNativeGatewayManifest.h` are emitted from the TypeScript capability records by `npm run registry:generate`. They are committed but must never be hand-edited.
