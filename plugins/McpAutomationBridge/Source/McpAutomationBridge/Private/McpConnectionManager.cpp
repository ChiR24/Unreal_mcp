...
void FMcpConnectionManager::Initialize(const UMcpAutomationBridgeSettings* Settings)
{
	if (Settings)
	{
...
void FMcpConnectionManager::Start()
{
	if (!TickerHandle.IsValid())
	{
		...
		const FTickerDelegate TickDelegate = FTickerDelegate::CreateLambda([WeakSelf](float DeltaTime) -> bool
		{
	...
	UE_LOG(LogMcpAutomationBridgeSubsystem, Log, TEXT("Starting MCP connection manager."));
	AttemptConnection();
}
...
void FMcpConnectionManager::Stop()
{
	...
	UE_LOG(LogMcpAutomationBridgeSubsystem, Log, TEXT("MCP connection manager stopped."));
}
...
bool FMcpConnectionManager::IsConnected() const
{
	...
	{
		if (Sock.IsValid() && Sock->IsConnected()) return true;
	}
	return false;
}

void FMcpConnectionManager::SetOnMessageReceived(FMcpMessageReceivedCallback InCallback)
{
...
{
	if (!bBridgeAvailable) return;

	UE_LOG(LogMcpAutomationBridgeSubsystem, Log, TEXT("AttemptConnection invoked."));
	...
	if (bShouldListen && !IsAnyServerListening())
	{
		...
		if (PortTokens.Num() == 0) PortTokens.Add(TEXT("8090"));
		if (!Settings->bMultiListen && PortTokens.Num() > 0) PortTokens.SetNum(1);
		...
		for (const FString& Token : PortTokens)
		{
			...
			int32 Port = 0;
			if (!LexTryParseString(Port, *Trimmed) || Port <= 0) continue;
			...
			ServerSocket->OnConnected().AddLambda([WeakSelf](TSharedPtr<FMcpBridgeWebSocket> Sock)
			{
				if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin())
				{
			...
			ServerSocket->OnClientConnected().AddLambda([WeakSelf](TSharedPtr<FMcpBridgeWebSocket> ClientSock)
			{
				if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin())
				{
			...
			ServerSocket->OnConnectionError().AddLambda([WeakSelf](const FString& Err)
			{
				if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin())
				{
	...
	if (!EndpointUrl.IsEmpty())
	{
		...
		if (!bHasClientForEndpoint)
		{
			...
			ClientSocket->OnConnected().AddLambda([WeakSelf](TSharedPtr<FMcpBridgeWebSocket> Sock)
			{
				if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin())
				{
					StrongSelf->HandleConnected(Sock);
			...
			ClientSocket->OnConnectionError().AddLambda([WeakSelf, ClientSocket](const FString& Err)
			{
				if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin())
				{
			...
			ClientSocket->OnMessage().AddLambda([WeakSelf](TSharedPtr<FMcpBridgeWebSocket> Sock, const FString& Message)
			{
				if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin())
				{
...
void FMcpConnectionManager::ForceReconnect(const FString& Reason, float ReconnectDelayOverride)
{
	UE_LOG(LogMcpAutomationBridgeSubsystem, Warning, TEXT("ForceReconnect: %s"), *Reason);
	...
	{
		TimeUntilReconnect = (ReconnectDelayOverride >= 0.0f) ? ReconnectDelayOverride : AutoReconnectDelaySeconds;
		// Re-enable bridge availability flag so tick will attempt connection after delay
		bBridgeAvailable = true;
	}
}

void FMcpConnectionManager::HandleConnected(TSharedPtr<FMcpBridgeWebSocket> Socket)
{
	if (!Socket.IsValid()) return;
	const int32 Port = Socket->GetPort();
	if (Socket->IsListening())
	{
		UE_LOG(LogMcpAutomationBridgeSubsystem, Log, TEXT("Automation bridge listening on port=%d"), Port);
	}
...
void FMcpConnectionManager::HandleClientConnected(TSharedPtr<FMcpBridgeWebSocket> ClientSocket)
{
	...
	// However, TFunction/Lambda binding with weak pointers is safer for async callbacks
	
	TWeakPtr<FMcpConnectionManager> WeakSelf = AsShared();
	
	ClientSocket->OnMessage().AddLambda([WeakSelf](TSharedPtr<FMcpBridgeWebSocket> Sock, const FString& Msg) {
		if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin()) StrongSelf->HandleMessage(Sock, Msg);
	});
	
	ClientSocket->OnClosed().AddLambda([WeakSelf](TSharedPtr<FMcpBridgeWebSocket> Sock, int32 Code, const FString& Reason, bool bClean) {
		if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin()) StrongSelf->HandleClosed(Sock, Code, Reason, bClean);
	});

	TWeakPtr<FMcpBridgeWebSocket> WeakSocket = ClientSocket;
	ClientSocket->OnConnectionError().AddLambda([WeakSelf, WeakSocket](const FString& Error)
	{
		if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin())
		{
	...
	ClientSocket->OnHeartbeat().AddLambda([WeakSelf](TSharedPtr<FMcpBridgeWebSocket> Sock) {
		if (TSharedPtr<FMcpConnectionManager> StrongSelf = WeakSelf.Pin()) StrongSelf->HandleHeartbeat(Sock);
	});
...
void FMcpConnectionManager::HandleConnectionError(TSharedPtr<FMcpBridgeWebSocket> Socket, const FString& Error)
{
...
void FMcpConnectionManager::HandleClosed(TSharedPtr<FMcpBridgeWebSocket> Socket, int32 StatusCode, const FString& Reason, bool bWasClean)
{
	const int32 Port = Socket.IsValid() ? Socket->GetPort() : -1;
...
void FMcpConnectionManager::HandleHeartbeat(TSharedPtr<FMcpBridgeWebSocket> Socket)
{
	LastHeartbeatTimestamp = FPlatformTime::Seconds();
	if (!bHeartbeatTrackingEnabled)
	{
		bHeartbeatTrackingEnabled = true;
		UE_LOG(LogMcpAutomationBridgeSubsystem, Verbose, TEXT("Heartbeat tracking enabled."));
	}
}

void FMcpConnectionManager::HandleMessage(TSharedPtr<FMcpBridgeWebSocket> Socket, const FString& Message)
{
	if (!Socket.IsValid()) return;
	...
	if (Type.Equals(TEXT("bridge_hello"), ESearchCase::IgnoreCase))
	{
		...
		Ack->SetStringField(TEXT("serverName"), !ServerName.IsEmpty() ? ServerName : TEXT("UnrealEditor"));
		Ack->SetStringField(TEXT("serverVersion"), !ServerVersion.IsEmpty() ? ServerVersion : TEXT("unreal-engine"));
...
void FMcpConnectionManager::SendControlMessage(const TSharedPtr<FJsonObject>& Message)
{
...
void FMcpConnectionManager::SendAutomationResponse(TSharedPtr<FMcpBridgeWebSocket> TargetSocket, const FString& RequestId, bool bSuccess, const FString& Message, const TSharedPtr<FJsonObject>& Result, const FString& ErrorCode)
{
	...
	if (!Message.IsEmpty()) Response->SetStringField(TEXT("message"), Message);
	if (!ErrorCode.IsEmpty()) Response->SetStringField(TEXT("error"), ErrorCode);
	if (Result.IsValid()) Response->SetObjectField(TEXT("result"), Result.ToSharedRef());
	...
	if (!bSent)
	{
		UE_LOG(LogMcpAutomationBridgeSubsystem, Warning, TEXT("Failed to deliver automation_response for RequestId=%s"), *RequestId);
		...
		if (Result.IsValid()) EventResult->SetObjectField(TEXT("payload"), Result.ToSharedRef());
...
void FMcpConnectionManager::RecordAutomationTelemetry(const FString& RequestId, bool bSuccess, const FString& Message, const FString& ErrorCode)
{
...
void FMcpConnectionManager::EmitAutomationTelemetrySummaryIfNeeded(double NowSeconds)
{
	if (TelemetrySummaryIntervalSeconds <= 0.0) return;
	if ((NowSeconds - LastTelemetrySummaryLogSeconds) < TelemetrySummaryIntervalSeconds) return;

	LastTelemetrySummaryLogSeconds = NowSeconds;
	if (AutomationActionTelemetry.Num() == 0) return;
...
void FMcpConnectionManager::RegisterRequestSocket(const FString& RequestId, TSharedPtr<FMcpBridgeWebSocket> Socket)
{
...
void FMcpConnectionManager::StartRequestTelemetry(const FString& RequestId, const FString& Action)
{
	if (!ActiveRequestTelemetry.Contains(RequestId))
	{
		FAutomationRequestTelemetry Entry;
		// Store lowercase action for consistent aggregation, similar to original logic
