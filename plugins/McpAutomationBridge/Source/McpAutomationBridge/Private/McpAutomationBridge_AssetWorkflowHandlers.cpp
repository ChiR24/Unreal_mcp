...
bool UMcpAutomationBridgeSubsystem::HandleGenerateReport(
  ...
  AsyncTask(ENamedThreads::GameThread, [this, RequestId, Socket, Directory,
    ...
    if (!OutputPath.IsEmpty()) {
      ...
      // Serialize the actual asset data to JSON for the report file
      TSharedPtr<FJsonObject> ReportJson = MakeShared<FJsonObject>();
      ReportJson->SetStringField(TEXT("generatedBy"), TEXT("MCP Automation Bridge"));
      ReportJson->SetStringField(TEXT("directory"), Directory);
      ReportJson->SetStringField(TEXT("reportType"), ReportType);
      ReportJson->SetNumberField(TEXT("assetCount"), AssetList.Num());
      ReportJson->SetStringField(TEXT("generatedAt"), FDateTime::Now().ToString());
      
      // Add detailed asset information
      TArray<TSharedPtr<FJsonValue>> DetailedAssets;
      int64 TotalDiskSize = 0;
      TMap<FString, int32> AssetsByClass;
      
      for (const FAssetData &Asset : AssetList) {
        TSharedPtr<FJsonObject> AssetDetail = MakeShared<FJsonObject>();
        AssetDetail->SetStringField(TEXT("name"), Asset.AssetName.ToString());
        AssetDetail->SetStringField(TEXT("path"), Asset.GetSoftObjectPath().ToString());
        AssetDetail->SetStringField(TEXT("class"), Asset.AssetClassPath.ToString());
        AssetDetail->SetStringField(TEXT("package"), Asset.PackageName.ToString());
        
        // Track stats by class
        FString ClassName = Asset.AssetClassPath.GetAssetName().ToString();
        AssetsByClass.FindOrAdd(ClassName)++;
        
        DetailedAssets.Add(MakeShared<FJsonValueObject>(AssetDetail));
      }
      ReportJson->SetArrayField(TEXT("assets"), DetailedAssets);
      
      // Add summary statistics
      TSharedPtr<FJsonObject> Stats = MakeShared<FJsonObject>();
      Stats->SetNumberField(TEXT("totalAssets"), AssetList.Num());
      
      TArray<TSharedPtr<FJsonValue>> ClassBreakdown;
      for (const auto& Pair : AssetsByClass) {
        TSharedPtr<FJsonObject> ClassStat = MakeShared<FJsonObject>();
        ClassStat->SetStringField(TEXT("class"), Pair.Key);
        ClassStat->SetNumberField(TEXT("count"), Pair.Value);
        ClassBreakdown.Add(MakeShared<FJsonValueObject>(ClassStat));
      }
      Stats->SetArrayField(TEXT("byClass"), ClassBreakdown);
      ReportJson->SetObjectField(TEXT("statistics"), Stats);
...
bool UMcpAutomationBridgeSubsystem::HandleGetMaterialStats(
  ...
  int32 InterpolatorCount = 0;
  
  // Try to get stats from the compiled material resource
  // UE 5.7+ uses EShaderPlatform, earlier versions use ERHIFeatureLevel
  ...
  if (const FMaterialResource* Resource = Material->GetMaterialResource(GMaxRHIFeatureLevel)) {
#endif
    // Get shader map to query instruction counts
    FMaterialShaderMap* ShaderMap = Resource->GetGameThreadShaderMap();
    if (ShaderMap) {
      // Iterate through shader types to find pixel shader
      TMap<FHashedName, TShaderRef<FShader>> Shaders;
      ShaderMap->GetShaderList(Shaders);
      
      for (const auto& Pair : Shaders) {
        const FShader* Shader = Pair.Value.GetShader();
        if (Shader) {
          // Get instruction count from shader
          int32 NumInstructions = Shader->GetNumInstructions();
          if (NumInstructions > InstructionCount) {
    ...
    // Alternative: get from compiled output
    if (InstructionCount == 0) {
      // Check if material has valid cached shader code
      TArray<FMaterialResourceLocOnDisk> OutMaterialResourcesLocOnDisk;
      // Fall back to expression count as rough estimate
      InstructionCount = -1; // Indicate not available at runtime
    }
  }
  
  Stats->SetNumberField(TEXT("instructionCount"), InstructionCount);
  if (InstructionCount == -1) {
    Stats->SetStringField(TEXT("instructionCountNote"), TEXT("Shader not compiled or instruction count unavailable at runtime"));
  }
