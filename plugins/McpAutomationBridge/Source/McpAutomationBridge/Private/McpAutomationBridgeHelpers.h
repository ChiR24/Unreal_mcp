...
static inline FString SanitizeProjectRelativePath(const FString &InPath) {
  ...
  // Validate against engine's registered mount points instead of a hardcoded whitelist.
  // This automatically supports /Game, /Engine, /Script, and all plugin mount points
  // (e.g., Game Feature Plugins like /ShooterCore/, /TopDownArena/, etc.)
  const bool bValidRoot = CleanPath.StartsWith(TEXT("/Script")) ||
                          FPackageName::IsValidLongPackageName(CleanPath, /*bIncludeReadOnlyRoots=*/true);

  if (!bValidRoot) {
    UE_LOG(
        LogMcpAutomationBridgeSubsystem, Warning,
        TEXT("SanitizeProjectRelativePath: Rejected path with unregistered mount point: %s"),
        *InPath);
    return FString();
  }
