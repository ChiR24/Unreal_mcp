...
#if WITH_EDITOR
void FSCSHandlers::FinalizeBlueprintSCSChange(UBlueprint* Blueprint, bool& bOutCompiled, bool& bOutSaved)
{
    bOutCompiled = false;
    bOutSaved = false;
...
TSharedPtr<FJsonObject> FSCSHandlers::GetBlueprintSCS(const FString& BlueprintPath)
{
    ...
    const FString& ParentComponentName)
{
    ...
    {
        Result->SetBoolField(TEXT("success"), false);
        Result->SetStringField(TEXT("error"), FString::Printf(TEXT("Verification failed: Component '%s' not found in SCS after add"), *ComponentName));
        return Result;
    }
...
TSharedPtr<FJsonObject> FSCSHandlers::RemoveSCSComponent(
    const FString& BlueprintPath,
    const FString& ComponentName)
{
    TSharedPtr<FJsonObject> Result = MakeShareable(new FJsonObject);
...
TSharedPtr<FJsonObject> FSCSHandlers::ReparentSCSComponent(
    const FString& BlueprintPath,
    const FString& ComponentName,
