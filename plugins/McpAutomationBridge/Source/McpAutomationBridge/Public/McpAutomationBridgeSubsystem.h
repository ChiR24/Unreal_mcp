...
public:
    ...
    /** Last target-window-relative pointer position used by bridge-owned simulated input. */
    FVector2D SimulatedPointerPosition = FVector2D::ZeroVector;

    /** Whether the simulated pointer position has been initialized. */
    bool bSimulatedPointerPositionValid = false;

    /** Currently pressed simulated mouse buttons tracked across requests by key name. */
    TSet<FString> SimulatedPressedMouseButtons;

    /** Currently pressed simulated modifier keys tracked across requests by key name. */
    TSet<FString> SimulatedPressedModifierKeys;

    /** Current live Slate window targeted by the bridge-owned virtual cursor session. */
    TWeakPtr<SWindow> SimulatedTargetWindow;

    /** Current live dock tab targeted by the bridge-owned virtual cursor session, when available. */
    TWeakPtr<SDockTab> SimulatedTargetDockTab;

    /** Exact tab id last used to resolve the current virtual cursor session. */
    FString SimulatedTargetTabId;

    /** Exact window title last used to resolve the current virtual cursor session. */
    FString SimulatedTargetWindowTitle;

    /** Single bridge-owned virtual cursor overlay attached to the current target window. */
    TSharedPtr<SWidget> SimulatedVirtualCursorOverlay;
