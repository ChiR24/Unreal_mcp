...
const testCases = [
  ...
  { scenario: 'Create animation blueprint minimal', toolName: 'animation_physics', arguments: { action: 'create_anim_blueprint', blueprintName: 'TC_AnimBP', parentClass: 'AnimInstance', targetSkeleton: '/Engine/Tutorial/SubEditors/TutorialAssets/Character/TutorialTPP_Skeleton', stateMachines: [] }, expected: 'success or not found' },
...
const testCases = [
...

  {
    ...
    arguments: { action: "create_animation_asset", assetName: "InvalidAnim", skeletonPath: "/Game/Invalid/Skeleton" },
    expected: "error|asset_not_found"
  },
  {
    scenario: "Edge: Play rate 0 (pause)",
    toolName: "animation_physics",
    arguments: { action: "play_montage", assetPath: "/Game/Animations/AM_Setup", playRate: 0 },
    expected: "success|no_op"
  },
  {
    scenario: "Border: Negative RPM vehicle",
    toolName: "animation_physics",
    arguments: { action: "configure_vehicle", vehicleName: "TestVehicle", maxRPM: -1000 },
    expected: "success|clamped"
  },
  {
    scenario: "Error: Empty wheels array",
    toolName: "animation_physics",
    arguments: { action: "configure_vehicle", vehicleName: "TestVehicle", wheels: [] },
    expected: "success|default_wheels"
  },
  {
    scenario: "Edge: No plugin deps",
    toolName: "animation_physics",
    arguments: { action: "configure_vehicle", vehicleName: "TestVehicle", plugins: ["InvalidPlugin"] },
