...
import {
  getBridgeBootstrapConfig,
  prepareLiveServerEnv,
  waitForAnyPort,
} from "./lib/bridge-bootstrap.mjs";
import {
  buildLiveSuiteCleanupOperations,
  buildManagedRunnerCleanupOperations,
  runBestEffortCleanup,
} from "./lib/live-suite-cleanup.mjs";
...
let serverArgs = process.env.UNREAL_MCP_SERVER_ARGS
...
const RESPONSE_LOGGING_ENABLED =
  process.env.UNREAL_MCP_TEST_LOG_RESPONSES !== "0";
...
function logMcpResponse(toolName, normalizedResponse) {
  const maxChars = Number(
    process.env.UNREAL_MCP_TEST_RESPONSE_MAX_CHARS ??
      DEFAULT_RESPONSE_LOG_MAX_CHARS,
  );
  const payload = {
...
function summarize(toolName, results, resultsPath) {
  ...
  if (totals.passed && totals.total > 0)
    console.log(
      `Pass rate: ${((totals.passed / totals.total) * 100).toFixed(1)}%`,
    );
...
function evaluateExpectation(testCase, response) {
  ...
  const containsFailure = failureKeywords.some((word) =>
    lowerExpected.includes(word),
  );
  const containsSuccess = successKeywords.some((word) =>
    lowerExpected.includes(word),
  );
  ...
  const primaryExpectsSuccess = successKeywords.some((word) =>
    primaryCondition.includes(word),
  );
  const primaryExpectsFailure = failureKeywords.some((word) =>
    primaryCondition.includes(word),
  );

  const structuredSuccess =
    typeof response.structuredContent?.success === "boolean"
      ? response.structuredContent.success
      : undefined;
  const actualSuccess =
    structuredSuccess ??
    !(response.isError || response.structuredContent?.isError);
  ...
  if (
    (response.isError === true ||
      response.structuredContent?.isError === true ||
  ...
  const hasInfrastructureError = infrastructureErrorCodes.some(
    (code) =>
  ...
  const hasDisconnect =
    /\bdisconnect(ed)?\b/i.test(errorStr) ||
    /\bdisconnect(ed)?\b/i.test(messageStr);
  const hasConnectionLost =
    /\bconnection lost\b/i.test(errorStr) ||
    /\bconnection lost\b/i.test(messageStr);
  const hasNotConnected =
    /\bnot connected\b/i.test(errorStr) ||
    /\bnot connected\b/i.test(messageStr);
  ...
  const crashIndicators = actualSuccess
    ? baseCrashIndicators
    : [...baseCrashIndicators, "disconnect"];
  const hasCrashIndicator =
    crashIndicators.some(
      (ind) =>
        errorStr.includes(ind) ||
        messageStr.includes(ind) ||
  ...
  const explicitlyExpectsCrash =
    lowerExpected.includes("disconnect") ||
  ...
  const hasTimeout =
    /\btimeout\b/i.test(combined) || combined.includes("timed out");
  const explicitlyExpectsTimeout =
    primaryCondition === "timeout" || primaryCondition.includes("timeout");
  ...
  if (
    ...
    !lowerExpected.includes("attachment failed")
  ) {
    return {
      passed: false,
  ...
  if (typeof expectation === "object" && expectation !== null) {
    ...
    if (actualSuccess && expectation.successPattern) {
      ...
      if (combined.includes(pattern)) {
        return {
          passed: true,
          reason: `Success pattern matched: ${expectation.successPattern}`,
        };
      }
    }
    ...
    if (!actualSuccess && expectation.errorPattern) {
      ...
      if (combined.includes(pattern)) {
        return {
          passed: true,
          reason: `Error pattern matched: ${expectation.errorPattern}`,
        };
      }
    }
  }
  ...
  if (lowerExpected.includes(" or ") || lowerExpected.includes("|")) {
    ...
    const conditions = lowerExpected
      .split(separator)
      .map((c) => c.trim())
      .filter(Boolean);
    for (const condition of conditions) {
      if (
        successKeywords.some((kw) => condition.includes(kw)) &&
        actualSuccess === true
      ) {
        return {
          passed: true,
      ...
      if (
        ...
        response.structuredContent.handled === true
      ) {
      ...
      if (
        (condition === "timeout" || condition.includes("timeout")) &&
        primaryCondition === condition
      ) {
        if (/\btimeout\b/i.test(combined) || combined.includes("timed out")) {
          return {
            passed: true,
            reason: `Expected timeout condition met: ${condition}`,
          };
        }
      }
      ...
      const conditionIsFailure = failureKeywords.some((kw) =>
        condition.includes(kw),
      );
      const conditionIsSuccess = successKeywords.some((kw) =>
        condition.includes(kw),
      );
      ...
      if (combined.includes(condition)) {
        if (conditionIsSuccess && actualSuccess === true) {
          return {
            passed: true,
            reason: `Expected condition met: ${condition}`,
          };
        }
        ...
        if (!conditionIsSuccess && !conditionIsFailure) {
          ...
          if (
            (primaryExpectsSuccess && actualSuccess === true) ||
            (primaryExpectsFailure && actualSuccess === false)
          ) {
            return {
              passed: true,
              reason: `Expected condition met: ${condition}`,
    ...
    return {
      passed: false,
      reason: `None of the expected conditions matched: ${expectedCondition}`,
    };
  }

  // Also flag common automation/plugin failure phrases
  // NOTE: "not_implemented" is acceptable only when a test explicitly expects it.
  ...
  const hasPluginFailure = pluginFailureIndicators.some((term) =>
  ...
  if (actualError === "UE_NOT_CONNECTED") {
    const explicitlyExpectsDisconnection =
      lowerExpected.includes("not connected") ||
      lowerExpected.includes("ue_not_connected") ||
  ...
  if (expectedFailure && !actualSuccess) {
    ...
    const expectedErrorType = specificErrorTypes.find((type) =>
      lowerExpected.includes(type.toLowerCase()),
    );
    ...
    const normalizedExpected = expectedErrorType
      ? expectedErrorType.toLowerCase().replace(/_/g, " ")
      : "";
    let errorTypeMatch = expectedErrorType
      ? normalizedReason.includes(normalizedExpected) ||
        lowerReason.includes(expectedErrorType.toLowerCase()) ||
        lowerReason.includes(expectedErrorType.toUpperCase())
      : failureKeywords.some(
          (keyword) =>
            lowerExpected.includes(keyword) && lowerReason.includes(keyword),
        );
    ...
    if (
      lowerExpected.includes("not_implemented") ||
      lowerExpected.includes("not found") ||
      lowerExpected.includes("not_found") ||
      lowerExpected.includes("invalid") ||
      lowerExpected.includes("missing") ||
      lowerExpected.includes("already exists") ||
      lowerExpected.includes("sc_disabled") ||
      lowerExpected.includes("security") ||
      lowerExpected.includes("blocked") ||
      lowerExpected.includes("violation") ||
      lowerExpected.includes("object_not_found") ||
      lowerExpected.includes("actor_not_found") ||
      lowerExpected.includes("actors not found") || // CRITICAL FIX: Handle plural form
      lowerExpected.includes("class_not_found") ||
      lowerExpected.includes("component_not_found") ||
      lowerExpected.includes("physics_failed") ||
      lowerExpected.includes("function_not_found") ||
      ...
      if (
        (lowerExpected.includes("actor_not_found") ||
          lowerExpected.includes("actors not found")) &&
        (normalizedReason.includes("actors not found") ||
          normalizedReason.includes("actor not found"))
      ) {
        return {
          passed: true,
          reason: `Expected error type matched (actor/actors not found): ${actualMessage || actualError}`,
        };
      }
      ...
      if (response.isError) {
        reason = response.content
          ?.map((entry) =>
            "text" in entry ? entry.text : JSON.stringify(entry),
          )
          .join("\n");
...
export async function runToolTests(toolName, testCases) {
        ...
        resolved[key] = value.replace(
          ...
          (match, captureKey) => {
            ...
            if (captured === undefined) {
              console.warn(
                `⚠️  Captured value '${captureKey}' not found, using placeholder`,
              );
              return match;
            }
            return captured;
  ...
  try {
    ...
    const { host: bridgeHost, ports: envPorts } = getBridgeBootstrapConfig(
      process.env,
    );
    ...
    console.log(
      `Waiting up to ${waitMs}ms for automation bridge on ${bridgeHost}:${envPorts.join(",")}`,
    );
    const discoveredListener = await waitForAnyPort(
      bridgeHost,
      envPorts,
      waitMs,
    );
    console.log(
      `✅ Automation bridge appears to be listening on ${discoveredListener.host}:${discoveredListener.port}`,
    );
    serverEnv = prepareLiveServerEnv(serverEnv, discoveredListener);
      ...
      try {
        ...
        if (srcIsNewer) {
          if (!autoBuildEnabled && !autoBuildDisabled) {
            console.log(
              "Detected newer source files than dist; attempting automatic build to refresh dist/ (set UNREAL_MCP_NO_AUTO_BUILD=1 to disable)",
            );
          }
          if (autoBuildEnabled || !autoBuildDisabled) {
            ...
            try {
              ...
              console.warn(
                "Automatic build failed or could not stat files — falling back to TypeScript source for live tests:",
                String(buildErr),
              );
              useDist = false;
            }
            ...
            console.log(
              "Detected newer source files than dist but automatic build is disabled.",
            );
            console.log(
              "Set UNREAL_MCP_AUTO_BUILD=1 to enable automatic builds, or run `npm run build` manually.",
            );
            useDist = false;
          }
        } else {
        ...
        useDist = false;
        console.log(
          "Preferring TypeScript source for tests to pick up local changes (set UNREAL_MCP_FORCE_DIST=1 to force dist)",
        );
      }
    } else {
      console.log(
        "dist not found — attempting to run `npm run build` to produce dist/ for live tests",
      );
      try {
        const { spawn } = await import("node:child_process");
        await new Promise((resolve, reject) => {
        ...
        console.warn(
          "Automatic build failed — falling back to running TypeScript source with ts-node-esm:",
          String(buildErr),
        );
        useDist = false;
      }
    ...
    callToolOnce = async function (callOptions, baseTimeoutMs) {
      const envDefault =
        Number(process.env.UNREAL_MCP_TEST_CALL_TIMEOUT_MS ?? "60000") || 60000;
      const perCall = Number(callOptions?.arguments?.timeoutMs) || undefined;
      const base =
        typeof baseTimeoutMs === "number" && baseTimeoutMs > 0
          ? baseTimeoutMs
          : perCall || envDefault;
      ...
      const clientTimeoutMs =
        Number(process.env.UNREAL_MCP_TEST_CLIENT_TIMEOUT_MS ?? "300000") ||
        300000;

      try {
        console.log(
          `[CALL] ${callOptions.name} (server timeout: ${serverTimeoutMs}ms, client timeout: ${clientTimeoutMs}ms)`,
        );
        const outgoing = Object.assign({}, callOptions, {
          arguments: {
        ...
        try {
          ...
          callPromise = client.callTool(outgoing, undefined, {
        ...
        const timeoutPromise = new Promise((_, rej) => {
          timeoutId = setTimeout(
            () =>
              rej(
                new Error(
                  `Local test runner timeout after ${clientTimeoutMs}ms`,
                ),
              ),
            clientTimeoutMs,
          );
        ...
        try {
          const timed = Promise.race([callPromise, timeoutPromise]);
          return await timed;
        } finally {
          if (timeoutId) {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (attempt < maxRetries) {
            ...
            const reason = isDriverCrash
              ? "driver instability detected"
              : "transient failure";
            console.warn(
              `  ⚠️  ${operationName} failed (attempt ${attempt}/${maxRetries}, ${reason}), retrying in ${delay}ms...`,
            );
    ...
    try {
      ...
      await callToolOnce(
        {
          name: "manage_level",
          arguments: { action: "unload", levelName: "MainLevel" },
        },
        10000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "manage_level",
          arguments: { action: "unload", levelName: "TestLevel" },
        },
        10000,
      ).catch(() => {});
      ...
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "TestCylinder" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "TestActor" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "ParentActor" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "ChildActor" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "NavTestActor" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "TestSpline" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "TestRoad" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "SplineControlPoints" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "NavLinkProxy_Test" },
        },
        5000,
      ).catch(() => {});
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "SmartNavLink_Test" },
        },
        5000,
      ).catch(() => {});
      ...
      await callToolOnce(
        {
          ...
          arguments: {
            action: "delete_asset",
            assetPath: "/Game/MCPTest/BP_Test",
          },
        },
        10000,
      ).catch(() => {});
      ...
      await callToolOnce(
        {
          ...
          arguments: {
            action: "delete_asset",
            assetPath: "/Game/MCPTest/TestMaterial",
          },
        },
        10000,
      ).catch(() => {});
      ...
      await callToolOnce(
        {
          ...
          arguments: {
            action: "delete_asset",
            assetPath: "/Game/MCPTest/ConvertedMesh",
          },
        },
        10000,
      ).catch(() => {});
      await callToolOnce(
        {
          ...
          arguments: {
            action: "delete_asset",
            assetPath: "/Game/MCPTest/TestLandscape",
          },
        },
        10000,
      ).catch(() => {});
      ...
      await callToolOnce(
        {
          ...
          arguments: {
            action: "delete_asset",
            assetPath: "/Game/MCPTest/TestAsset",
          },
        },
        10000,
      ).catch(() => {});
      ...
      await callToolOnce(
        {
          ...
          arguments: {
            action: "delete_asset",
            assetPath: "/Game/MCPTest/TestMat",
          },
        },
        10000,
      ).catch(() => {});
      await callToolOnce(
        {
          ...
          arguments: {
            action: "delete_asset",
            assetPath: "/Game/MCPTest/TestInstance",
          },
        },
        10000,
      ).catch(() => {});
      ...
      await callToolOnce(
        {
          name: "control_actor",
          arguments: { action: "delete", actorName: "NavMeshBoundsVolume" },
        },
        5000,
      ).catch(() => {});
    ...
    try {
      ...
      await callWithRetry(
        ...
        { maxRetries: 3, timeoutMs: 20000, operationName: "spawn TestActor" },
      ).catch((err) =>
        console.warn("⚠️  TestActor may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  Physics setup for TestActor failed:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  ParentActor may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  ChildActor may already exist:", err?.message || err),
      );
      ...
      await callWithRetry(
        ...
        },
        {
          maxRetries: 3,
          timeoutMs: 20000,
          operationName: "create BP_Test blueprint",
        },
      ).catch((err) =>
        console.warn("⚠️  BP_Test may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  TestMaterial may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  Parent material may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  M_Test material may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  TestMat material may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  TestInstance material instance may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestAsset may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestMesh may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  Grass foliage type may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  Tree foliage type may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  Bush foliage type may already exist:",
          err?.message || err,
        ),
      );
      // === End Foliage Setup ===
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestBox may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestSphere may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  TestCylinder may already exist:",
          err?.message || err,
        ),
      );
      // === End Geometry Setup ===
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  TestNavMeshBounds may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  Navigation rebuild may have failed:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  NavModifier component may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        ...
        console.warn(
          "⚠️  NavTestActor may already exist:",
          err?.message || err,
        ),
      );
      ...
      await callToolOnce(
        {
          ...
          arguments: {
            ...
            actorName: "TestSpline",
            location: { x: 0, y: 0, z: 0 },
          },
        },
        15000,
      ).catch((err) =>
        console.warn("⚠️  TestSpline may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestRoad may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestRiver may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestFence may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestWall may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        {
          name: "manage_splines",
          arguments: {
            action: "create_cable_spline",
            actorName: "TestCable",
            location: { x: 2500, y: 0, z: 100 },
          },
        },
        15000,
      ).catch((err) =>
        console.warn("⚠️  TestCable may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  TestPipe may already exist:", err?.message || err),
      );
      ...
      await callToolOnce(
        ...
        console.warn("⚠️  SplineBP may already exist:", err?.message || err),
      );
      // === End Spline Setup ===
      ...
      await callWithRetry(
        ...
        { maxRetries: 5, timeoutMs: 20000, operationName: "create TestLevel" },
      ).catch((err) =>
        console.warn(
          "⚠️  TestLevel creation failed after retries:",
          err?.message || err,
        ),
      );
      ...
      await callWithRetry(
        ...
        { maxRetries: 5, timeoutMs: 20000, operationName: "create MainLevel" },
      ).catch((err) =>
        console.warn(
          "⚠️  MainLevel creation failed after retries:",
          err?.message || err,
        ),
      );
      ...
      console.warn(
        "⚠️  Test asset setup had issues (tests may fail if assets missing):",
    ...
    const TEST_THROTTLE_MS = Number(
      process.env.UNREAL_MCP_TEST_THROTTLE_MS ?? 100,
    );
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      const testCaseTimeoutMs = Number(
        process.env.UNREAL_MCP_TEST_CASE_TIMEOUT_MS ??
          testCase.timeoutMs ??
          testCase.arguments?.timeoutMs ??
          "5000",
      );
      ...
      try {
        ...
        const cleanScenario = (testCase.scenario || "Unknown Test").replace(
          /"/g,
          "'",
        );
        await callToolOnce(
          {
            ...
            arguments: {
              ...
              command: `Log "---- STARTING TEST: ${cleanScenario} ----"`,
            },
          },
          5000,
        ).catch(() => {});
      ...
      try {
        ...
        const response = await callToolOnce(
          { name: testCase.toolName, arguments: resolvedArgs },
          testCaseTimeoutMs,
        );
        ...
        if (RESPONSE_LOGGING_ENABLED) {
          logMcpResponse(
            testCase.toolName + " :: " + testCase.scenario,
            normalizeMcpResponse(normalizedResponse),
          );
        }
        let { passed, reason } = evaluateExpectation(
          testCase,
          normalizedResponse,
        );
        ...
        const isPerformanceTest =
          testCase.arguments?.timeoutMs !== undefined ||
        ...
        const lowerExpected = (testCase.expected || "")
          .toString()
          .toLowerCase();
        const primaryCondition = lowerExpected
          .split("|")[0]
          .split(" or ")[0]
          .trim();
        const primaryExpectsFailure = failureKeywords.some((word) =>
        ...
        if (
          ...
          passed = false;
          const errorMsg =
            normalizedResponse?.structuredContent?.error ||
        ...
        if (!passed) {
          console.log(
            `[FAILED] ${testCase.scenario} (${durationMs.toFixed(1)} ms) => ${reason}`,
          );
          results.push({
          ...
          console.log(
            `[PASSED] ${testCase.scenario} (${durationMs.toFixed(1)} ms)`,
          );
          results.push({
            scenario: testCase.scenario,
            toolName: testCase.toolName,
        ...
        const isCrashError = crashIndicators.some((ind) =>
          lowerError.includes(ind),
        );
        const explicitlyExpectsCrash =
          lowerExpected.includes("disconnect") ||
          lowerExpected.includes("crash") ||
        ...
        if (isCrashError && !explicitlyExpectsCrash) {
          console.log(
            `[FAILED] ${testCase.scenario} (${durationMs.toFixed(1)} ms) => CRASH/CONNECTION LOSS: ${errorMessage}`,
          );
          results.push({
            scenario: testCase.scenario,
            toolName: testCase.toolName,
        ...
        const primaryCondition = lowerExpected
          ...
          .split(" or ")[0]
          .trim();
        const primaryExpectsTimeout =
          primaryCondition === "timeout" ||
          primaryCondition.includes("timeout");
        ...
        console.log(
          `[FAILED] ${testCase.scenario} (${durationMs.toFixed(1)} ms) => Error: ${errorMessage}`,
        );
        results.push({
          scenario: testCase.scenario,
          toolName: testCase.toolName,
      ...
      const isHighImpactOp =
        isGeometryTest && HIGH_IMPACT_OPS.some((op) => testAction.includes(op));
      if (isHighImpactOp) {
        console.log(
          "  🔄 Resetting geometry before high-impact operation: " + testAction,
        );
        ...
        if (geometryResetCounter % GEOMETRY_RESET_INTERVAL === 0) {
          console.log(
            "  🔄 Resetting geometry actors to prevent polygon accumulation...",
          );
          await resetGeometryActors();
        }
      }
  ...
  async run() {
    ...
    try {
      ...
      const waitMs = parseInt(
        process.env.UNREAL_MCP_WAIT_PORT_MS ?? "5000",
        10,
      );
      let serverEnv = Object.assign({}, process.env);
            ...
            if (autoBuildEnabled || !autoBuildDisabled) {
              ...
              try {
                await new Promise((resolve, reject) => {
                  const npmCmd =
                    process.platform === "win32" ? "npm.cmd" : "npm";
                  const ps = spawn(npmCmd, ["run", "build"], {
                    cwd: repoRoot,
                    stdio: "inherit",
      ...
      if (!useDist) {
        ...
        serverCommand = process.env.UNREAL_MCP_SERVER_CMD ?? serverCommand;
        serverArgs =
          process.env.UNREAL_MCP_SERVER_ARGS?.split(",") ?? serverArgs;
      }
      ...
      callToolOnce = async function (callOptions, baseTimeoutMs) {
        const envDefault =
          Number(process.env.UNREAL_MCP_TEST_CALL_TIMEOUT_MS ?? "60000") ||
          60000;
        const perCall = Number(callOptions?.arguments?.timeoutMs) || undefined;
        const base =
          typeof baseTimeoutMs === "number" && baseTimeoutMs > 0
        ...
        try {
          ...
          const outgoing = Object.assign({}, callOptions, {
            arguments: { ...(callOptions.arguments || {}), timeoutMs },
          });
          let callPromise;
          try {
            callPromise = client.callTool(outgoing, undefined, {
          ...
          const timeoutPromise = new Promise((_, rej) => {
            timeoutId = setTimeout(
              () =>
                rej(
                  new Error(`Local test runner timeout after ${timeoutMs}ms`),
                ),
              timeoutMs,
            );
            if (timeoutId && typeof timeoutId.unref === "function") {
      ...
      for (const step of this.steps) {
          ...
          await callToolOnce(
            {
              ...
              arguments: {
                action: "console_command",
                command: `Log "---- STARTING STEP: ${cleanName} ----"`,
              },
            },
            5000,
          ).catch(() => {});
        ...
        try {
          const rawResult = await step.fn(tools);
          const stepResult = this.normalizeStepResult(rawResult);
          const durationMs = performance.now() - startTime;
          const status = stepResult.passed ? "passed" : "failed";
          const detail =
            stepResult.detail ??
            (stepResult.passed ? "" : "Step returned false");
          console.log(
            formatResultLine(
              { scenario: step.name },
              status,
