// tests/unit/task-52/certification-verdict.test.ts
...
describe('judgeTreeStability — the shared-worktree injection', () => {
  const recorded = [
    { path: 'tests/unit/task-52/engine-inventory.mjs', sha256: 'aaa' },
    { path: 'scripts/qa/task52-certify-engine.mjs', sha256: 'bbb' },
  ];
  ...
  it('names the file another lane changed mid-run, and the stage it changed by', () => {
    ...
    expect(verdict.moved.map((entry) => entry.path)).toEqual(['scripts/qa/task52-certify-engine.mjs']);
