 ...
 *
 * These tests pin the reviewed, evidence-derived metrics (1,288 occurrences;
 * 36 duplicate names; 83 duplicate-name occurrences; max 47 exact-name
 * reductions; 801 add/create/set/configure) and the structural invariants the
...
describe('reviewed fixed metrics (reproduced from authoritative source)', () => {
  ...
  it('has exactly 1,288 classified occurrences', () => {
    expect(inv.occurrences.length).toBe(1288);
    expect(inv.metrics.occurrenceCount).toBe(1288);
  });
  ...
  it('reports 801 add/create/set/configure verb-family occurrences', () => {
    expect(inv.metrics.verbFamilyAddCreateSetConfigure).toBe(801);
  });
  ...
  it('derives 1,283 canonical definitions (5 true-duplicate reductions, under the 47 ceiling)', () => {
    expect(inv.canonicalDefinitions.length).toBe(1283);
