...
describe('canonical record loader preserves authored order', () => {
  it('loads exactly 1,383 unique sha256-hashed records', () => {
    expect(ALL_CAPABILITY_RECORD_COUNT).toBe(1400);
    expect(ALL_CAPABILITY_RECORDS).toHaveLength(1400);
    expect(new Set(ALL_CAPABILITY_RECORDS.map((record) => record.id)).size).toBe(1400);
