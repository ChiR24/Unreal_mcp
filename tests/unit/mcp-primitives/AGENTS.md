...
## SCOPE
27 `.test.ts` files in `tests/unit/mcp-primitives/` (5 named `*-parity.test.ts`). No subdirectories. Each file covers one primitive family and uses a GREEN/RED guard pattern (see below).
