...
describe('core capability retrieval aggregate', () => {
  it('contains exactly 180 unique hashed records in canonical order', () => {
    const catalog = createCoreCapabilityCatalog(COMPLETE_CORE_SOURCES);
    const ids = catalog.map((record) => record.id);

    expect(CORE_CAPABILITY_RECORD_COUNT).toBe(180);
    expect(catalog).toHaveLength(180);
    expect(new Set(ids)).toHaveLength(180);
  ...
  it('exports a frozen core catalog without changing the frozen pilot catalog', () => {
    ...
    expect(CORE_CAPABILITY_CATALOG).toHaveLength(180);
