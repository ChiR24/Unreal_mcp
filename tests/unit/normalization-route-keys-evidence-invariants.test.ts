...
/**
 * v2 deterministic expected key set (67 non-public routes).
...
const V2_EXPECTED_ROUTE_KEYS: readonly string[] = [
  ...
  'route:geometry:difference', 'route:geometry:bridge', 'route:geometry:loft', 'route:geometry:dynamicmesh_raw_residual',
...
describe('route dispositions: v2 exact-key completeness (67 non-public routes)', () => {
  const inv = buildInventory();
  const keys = inv.routeDispositions.map((r) => r.dispositionKey);
  const keySet = new Set(keys);

  it('has exactly 67 non-public route dispositions (v2 expected total)', () => {
    expect(inv.routeDispositions.length).toBe(67);
    expect(REVIEWED_ROUTE_KEYS.length).toBe(67);
  });
...
describe('route dispositions: v2 aggregate metrics (internally consistent totals)', () => {
  ...
  it('byDomain matches v2: widget23/graph1/skeleton16/animation4/gas5/ai4/effect3/geometry4/audio4/asset3', () => {
    ...
    expect(byDomain).toEqual({
      ...
      ai: 4, effect: 3, geometry: 4, audio: 4, asset: 3,
    });
  });

  it('byStatus matches v2: dead20/raw14/hidden33 (sum=67)', () => {
    expect(m.routeStatusCounts).toEqual({ dead: 20, raw: 14, hidden: 33 });
    expect(m.routeStatusCounts.dead + m.routeStatusCounts.raw + m.routeStatusCounts.hidden).toBe(67);
  });

  it('byDisposition matches v2: promote43/map16/remove8 (sum=67)', () => {
    expect(m.routeDispositionCounts).toEqual({ promote: 43, map: 16, remove: 8 });
    expect(m.routeDispositionCounts.promote + m.routeDispositionCounts.map + m.routeDispositionCounts.remove).toBe(67);
  });
