...
describe('buildUnbuiltRootBlocker', () => {
  const record = buildUnbuiltRootBlocker({
    minorKey: '5.0',
    versionString: '5.0.3',
    detection: { command: 'node scripts/qa/task56-evidence.mjs', detectedAt: '2026-07-28T00:00:00.000Z' },
    rootReports: [{
  ...
  it('names an acceptance command that would falsify it', () => {
    expect(record.requiredOperatorInput.acceptanceCommand).toContain('task52-certify-engine.mjs');
