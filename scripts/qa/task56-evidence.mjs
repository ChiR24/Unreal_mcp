...
//   disturbed.
//
// Run: node scripts/qa/task56-evidence.mjs [--out FILE]
...
import { judgeCleanupRelease } from '../../tests/unit/task-52/certification-verdict.mjs';
import { evaluateCapability, defineProfile } from '../../tests/unit/task-52/profile-matrix.mjs';
import { judgeOwnership } from '../../tests/unit/task-52/disposable-project.mjs';
import {
  buildUnbuiltRootBlocker, diagnoseBuildToolchain, judgeCertificationReadiness, probeEngineRoot,
} from '../../tests/unit/task-56/engine-readiness.mjs';
...
const TREE = [
  ...
  'tests/unit/task-56/engine-readiness.test.ts',
  'scripts/qa/task56-evidence.mjs',
...
aggregator.recordArtifact({
  path: '.omo/evidence/task-56/certify-5.0-attempt.json',
  inputsNewest: 'scripts/qa/task52-certify-engine.mjs',
  inputsNewestAtMs: statSync(join(REPO, 'scripts/qa/task52-certify-engine.mjs')).mtimeMs,
});
...
aggregator.document.environment.blocker = buildUnbuiltRootBlocker({
  ...
  rootReports,
  detection: {
    command: 'node scripts/qa/task52-certify-engine.mjs --engine-version 5.0',
