...
//
// Run: node scripts/qa/task52-certify-engine.mjs --engine-version 5.7 [--out FILE] [--keep]

import { copyFileSync, existsSync, mkdirSync } from 'node:fs';
import { basename, dirname, join, relative } from 'node:path';

import { buildEngineInventory, formatInventoryTable } from '../../tests/unit/task-52/engine-inventory.mjs';
import { DisposableWorkspace, surveyOwnedParent } from '../../tests/unit/task-52/disposable-project.mjs';
...
import { runCertificationDrivers } from '../../tests/unit/task-52/certification-drivers.mjs';
import {
  judgeCleanupAgreement, judgeCleanupRelease, judgeEditorLiveness, judgeProcessRelease, judgeTreeStability,
} from '../../tests/unit/task-52/certification-verdict.mjs';
import { EvidenceAggregator, identifyEngine } from '../../tests/unit/task-50/evidence-aggregator.mjs';
...
const OUT = argOf('--out', `.omo/evidence/task-52/certify-${MINOR}.json`);
...
async function main() {
  ...
  try {
    ...
    const keptArchive = join(REPO, '.omo/evidence/task-52/artifacts', `${workspace.runId}-${basename(packaged.archive)}`);
