...
import { runLoad, OWNED_WS_PORTS, processAlive, summariseRss } from '../../tests/unit/task-51/load-harness.mjs';
import { runSoak, runProcessResidueSoak } from '../../tests/unit/task-51/soak-harness.mjs';
import { runProtocolFuzz } from '../../tests/unit/task-51/protocol-fuzz-harness.mjs';
import { BUDGETS, RSS_LIMITS, SEEDS, TIME_LIMITS_MS } from '../../tests/unit/task-51/fuzz-seeds.mjs';
...
function parseArgs(argv) {
  ...
  const out = {
    out: '.omo/evidence/task-51/adversarial-run.json',
...
main().catch((error) => {
  process.stderr.write(`task51-adversarial failed: ${error instanceof Error ? error.stack : String(error)}\n`);
  process.exitCode = 1;
});
