#!/usr/bin/env node
// scripts/qa/native-capture.mjs
// Task 38 lane E — the OWNED runner that drives the executable native-protocol
// capture seam (Private/Tests/NativeProtocolCaptureTests.cpp) and feeds its
...
//   --out=<path>                  report path (must stay under .omo/evidence/mcp-primitives/harness-)
//   --json-only / --help
//
...
} from '../../tests/unit/mcp-primitives/parity-harness.mjs';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = resolve(HERE, '../..');
const FIXTURES_DIR = join(REPO_ROOT, 'tests/fixtures/mcp-primitives');
const OWNED_SCRATCH = '/tmp/opencode';
const DEFAULT_OUT = join(REPO_ROOT, '.omo/evidence/mcp-primitives/harness-native-capture.json');
...
function printHelp() {
  process.stdout.write(`native-capture — runner for the executable native-protocol capture seam

Usage: node scripts/qa/native-capture.mjs [--plan|--verify|--execute] [options]
...
function main() {
  ...
  const confined = outRel.startsWith(join('.omo', 'evidence', 'task-38', 'harness-')) || outRel.includes('.omo/evidence/mcp-primitives/harness-');
  if (confined) {
  ...
  if (!opts.jsonOnly) {
    process.stdout.write(`native-capture: mode=${report.mode} status=${report.status} crossTransportParity=${report.crossTransportParity}\n`);
