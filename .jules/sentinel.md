...
## 2026-04-26 - [Command Injection via Console Command Arguments]\n**Vulnerability:** In `src/tools/handlers/system-handlers.ts`, the `set_cvar` handler interpolated user-controlled input (`rawName` and `value`) directly into a string that was executed via the `console_command` action without sanitization. This allowed attackers to break out of the command and execute arbitrary other Unreal Engine console commands by supplying strings like `value; Quit;`.\n**Learning:** Just because an action expects a specific type of input (e.g., CVar name and value), if it ultimately constructs a console command string, all dynamically inserted parts must be treated as untrusted and sanitized. Command injection in console commands is a severe vector for arbitrary execution inside Unreal Engine.\n**Prevention:** Apply `sanitizeCommandArgument` to all parts of user input before interpolating them into console command strings. Ensure the sanitized parts are checked for emptiness before use, and return the sanitized values in the JSON response to maintain consistency.
