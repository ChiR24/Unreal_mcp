...
## 2026-01-26 - [Command Injection via Argument Interpolation in LevelTools]
**Vulnerability:** `LevelTools` constructed console commands (e.g., `CreateSubLevel`, `SetGameMode`) by interpolating identifiers or paths without validation. This allowed command injection (using `;`) or argument injection (using spaces). `sanitizeAssetName` was available but broken for paths (prepends `Asset_` to `/`), and `sanitizeConsoleString` was insufficient for unquoted arguments (leaves `;`).
**Learning:** Generic sanitizers (`sanitizeAssetName`) may have business logic (like prefixes) that make them unsuitable for raw paths or command arguments. Console commands that do not use quotes for arguments are vulnerable to space-based argument injection.
**Prevention:** Use a dedicated `sanitizeCommandArgument` utility for identifiers/paths that strictly allowlists characters (`a-zA-Z0-9_./-`) and replaces everything else (including `;`, spaces, quotes) to prevent breakout.
