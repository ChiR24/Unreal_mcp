...
- **Audit bar**: the blocking audit is runtime-only at `high`. That level dates from when GHSA-frvp-7c67-39w9 sat on the production path under the then-pinned `@modelcontextprotocol/sdk` 1.29.0; it is now cleared (SDK 1.30.0 + `@hono/node-server` off 1.x) and **both audits report zero** against this lockfile. Tightening to `moderate` is therefore possible but is a deliberate change: the command string is pinned by `tests/unit/workflow_gate_order_contract.test.ts` — see `docs/security-and-receipts.md`.
