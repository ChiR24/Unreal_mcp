...
- **`manage_blueprint: set_node_property` refreshes stale node title cache for `K2Node_EnhancedInputAction`** – writing to a property whose value contributes to `GetNodeTitle` (e.g. `InputAction` on an EnhancedInputAction K2Node) used to leave the editor displaying the stale title (`EnhancedInputAction None`) until the user manually clicked the node, even though the binding itself was applied correctly. Adds a narrow class-name whitelist in `HandleBlueprintGraphAction` that, when matched, calls `K2Node::ReconstructNode()` and the schema's `ForceVisualizationCacheClear()` *before* `NotifyGraphChanged` / `MarkBlueprintAsModified` / `SaveLoadedAssetThrottled`, so the reconstructed pin layout is the version that gets notified, dirtied, and persisted (no reliance on a subsequent compile to reconcile state). Class match is by string name to avoid pulling `InputBlueprintNodes` in as a hard plugin dependency. Other K2Node classes are unaffected — connected pins are not rebuilt.
