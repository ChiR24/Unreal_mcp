...
| Link failed with `LNK1194: cannot delay-load 'UnrealEditor-PCG.dll' due to import of data symbol PCG::Private::UserParameterTagData` on UE 5.8. MSVC cannot delay-load a DLL from which data is imported. | PCG is linked normally instead of delay-loaded, and the `PCG` plugin dependency in `McpAutomationBridge.uplugin` is no longer optional, so it is always mounted before the bridge loads. |
