...
export const MATERIAL_GRAPH_RECORDS: readonly RecordSpec[] = [
  ...
  r('connect_material_pins', 'material', 'Connect material pins (alias of connect_nodes).', schema({ materialPath: MAT, sourceNodeId: str('Source node ID.'), sourcePin: str('Source pin name.'), targetNodeId: str('Target node ID.'), targetPin: str('Target pin name.') }, ['materialPath', 'sourceNodeId', 'targetNodeId']), OK, WRITE, WRITE_POLICY, LOW,
  ...
  r('set_static_switch_parameter_value', 'material', 'Set a static switch parameter value on a material instance.', schema({ instancePath: str('Material instance /Game path.'), parameterName: str('Parameter name.'), value: bool('Switch value.') }, ['instancePath', 'parameterName', 'value']), OK, WRITE, WRITE_POLICY, LOW,
    { dispatchMode: 'tool', examples: [ex('Enable a detail-map switch on an instance', { instancePath: '/Game/Materials/MI_Base_Rusty', parameterName: 'UseDetailMap', value: true }, DONE)] }),
  ...
  r('set_material_parameter', 'material', 'Set a material parameter value.', schema({ materialPath: MAT, parameterName: str('Parameter name.'), value: { description: 'Parameter value.' } }, ['materialPath', 'parameterName']), OK, WRITE, WRITE_POLICY, LOW,
    { dispatchMode: 'tool', examples: [ex('Set a roughness parameter', { materialPath: M, parameterName: 'Roughness', value: 0.35 }, DONE)] }),
