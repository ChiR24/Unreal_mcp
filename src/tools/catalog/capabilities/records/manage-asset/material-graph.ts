...
export const MATERIAL_GRAPH_RECORDS: readonly RecordSpec[] = [
  ...
  r('update_custom_expression', 'material', 'Update a custom expression node: its HLSL code, inputs, output type, additional outputs or title. An input that keeps its name keeps its wire.', schema({ materialPath: MAT, nodeId: str('Node ID.'), code: str('Updated HLSL code.'), inputs: arrObj('Replacement input list, each {name}; inputs whose names stay keep their connections.'), outputType: str('Output type: Float1, Float2, Float3, Float4 or MaterialAttributes.'), description: str('Node title shown in the material editor.'), additionalOutputs: arrObj('Additional named output definitions.') }, ['materialPath', 'nodeId', 'code']), OK, WRITE, WRITE_POLICY, LOW,
